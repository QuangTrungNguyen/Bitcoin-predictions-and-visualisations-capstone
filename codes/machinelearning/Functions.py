...
def wrapper_feature_selector(X,Y,model,subset):
    sel = subset.copy() # # Selected features
    overall_error = train_and_predict(X[:,sel],Y,model,predict=False,return_y=False) 
    while len(sel) != 0:
        # Select candidate
        ...
        for cand in sel:
            ...
            if len(features) > 1:
                new_error = train_and_predict(X[:,features],Y,model,predict=False,return_y=False)
            else:
                new_error = train_and_predict(X[:,features[0]].reshape(-1,1),Y,model,predict=False,return_y=False)
    ...
    rmse = train_and_predict(X[:,sel],Y,model,predict=False,return_y=False)
    return [sel, rmse]
